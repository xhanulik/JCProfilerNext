...
* `spaTime` -- Measure elapsed time via SPA.
...
SPA Time Example
~~~~~~~~~~~~
Consider the `example` method in the `Example` class in the Time Example section.  The outer loop is clearly
not constant since its iteration count depends on the input value.  On the other
hand, the inner loop is constant.

Instrument, compile and install applet via card reader:
[source,console]
----
$ ./gradlew run --args="--work-dir example --jckit jc222_kit --executable example --ins 0xEE --data-regex 00[0-9A-F]{2} --repeat-count 100 --mode spaTime --delimiter ./delimiter_path.csv"
----
JCProfilerNext will process all sources in the `example` directory and will
also use this directory to store all intermediate results as by the original time profiling.
...
This is a modified version of JCProfilerNext, which is licensed under the GNU General Public License v3.0.

Original project URL: https://github.com/lzaoral/JCProfilerNext

Modifications have been made by Veronika Hanulikova as part of a Master's Thesis at Masaryk University in 2025.

All modified files contain notices of the changes.
